...
func Test_IsNTLMToken(t *testing.T) {
	...
	assert.True(t, httpauth.IsNTLMToken(ntlmToken))
	...
	assert.False(t, httpauth.IsNTLMToken(nonNtlmToken))
}

func Test_GetMechanismsFromToken_NTLM(t *testing.T) {
	ntlmToken := ""
	expected := "NTLM"
	actual, err := httpauth.GetMechanismsFromToken(ntlmToken)
	assert.Nil(t, err)
	assert.Contains(t, actual, expected)
}

func Test_GetMechanismsFromToken_NTLM_Negotiate(t *testing.T) {
...
func Test_GetMechanismsFromToken_Multiple(t *testing.T) {
	...
	expected := string(gssapi.OIDSPNEGO)
	actual, err := httpauth.GetMechanismsFromToken(ntlmToken)
	assert.Nil(t, err)
	assert.Contains(t, actual, expected)
}
